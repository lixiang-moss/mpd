    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.model, GaussianDiffusionModel):
            diffusion_sampling_method = args_inference.diffusion_sampling_method
            diffusion_sampling_cfg_key = diffusion_sampling_method
            # Allow selecting a DPM-Solver++ variant without requiring a separate YAML section.
            if diffusion_sampling_method == "dpm_solver_pp" and diffusion_sampling_cfg_key not in args_inference:
                diffusion_sampling_cfg_key = "dpm_solver"

            diffusion_sampling_args = args_inference[diffusion_sampling_cfg_key]
            if diffusion_sampling_method == "ddpm":
                t_start_guide = ceil(
                    diffusion_sampling_args.t_start_guide_steps_fraction * self.model.n_diffusion_steps
                )
            elif diffusion_sampling_method == "ddim":
                t_start_guide = ceil(
                ...
                t_start_guide = ceil(
                    diffusion_sampling_args.t_start_guide_steps_fraction * diffusion_sampling_args.dpm_solver_steps
                )
                # Set/force sensible defaults for the selected variant.
                # NOTE: algorithm_type is the DPM-Solver paper's "dpmsolver" vs "dpmsolver++".
                if diffusion_sampling_method == "dpm_solver_pp":
                    diffusion_sampling_args.dpm_solver_algorithm_type = "dpmsolver++"
                elif "dpm_solver_algorithm_type" not in diffusion_sampling_args:
                    diffusion_sampling_args.dpm_solver_algorithm_type = "dpmsolver++"
            elif diffusion_sampling_method == "unipc":
                t_start_guide = ceil(
                    diffusion_sampling_args.t_start_guide_steps_fraction * diffusion_sampling_args.unipc_steps
                )
                if "unipc_skip_type" not in diffusion_sampling_args:
                    diffusion_sampling_args.unipc_skip_type = "logSNR"
                if "unipc_variant" not in diffusion_sampling_args:
                    diffusion_sampling_args.unipc_variant = "bh1"
            else:
                raise ValueError

            diffusion_sampling_args.update(
                method=diffusion_sampling_method,
