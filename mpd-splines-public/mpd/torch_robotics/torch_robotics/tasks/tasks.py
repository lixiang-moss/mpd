        ...
        if trajs_valid_idxs.nelement() == 0:
            ...
            if filter_joint_limits_vel_acc:
                if self.robot.dq_max is not None:
                    ...
                    # Some trajectories contain only q_pos (no velocity/acc in the state).
                    # In that case, skip velocity limit checks.
                ...
                if self.robot.ddq_max is not None:
                    ...
                    # Some trajectories contain only q_pos (no velocity/acc in the state).
                    # In that case, skip acceleration limit checks.
    ...
    def animate_robot_trajectories(
        ...
        **kwargs,
        ...
        def animate_fn(i, ax):
            ...
            for q in qs:
                # Pass q as a positional argument to support different robot.render(...) signatures
                # (some robots use `q`, others use `q_pos`).
                self.robot.render(
                    ax,
                    q,
                    color=(
