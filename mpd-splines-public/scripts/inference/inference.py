    ...
    **kwargs,
    ...
    for idx_sg, idx_sample in enumerate(idx_sample_l):
        ...
        if save_results_single_plan_low_mem:
            # NOTE:
            # - Full results_single_plan includes full diffusion chains (iters) for all samples, which can be huge
            #   when doing large sweeps (e.g., steps=80, n_trajectory_samples=100).
            # - For sweeps we typically only need timing + metrics (+ optionally the best trajectory for debugging).
            results_single_plan_to_save = DotMap(
                t_generator=getattr(results_single_plan, "t_generator", None),
                t_guide=getattr(results_single_plan, "t_guide", None),
                t_inference_total=getattr(results_single_plan, "t_inference_total", None),
                q_pos_start=q_pos_start,
                q_pos_goal=q_pos_goal,
                ee_pose_goal=ee_pose_goal,
                metrics=getattr(results_single_plan, "metrics", None),
                isaacgym_statistics=getattr(results_single_plan, "isaacgym_statistics", None),
                # Keep only the best trajectory/control points (tiny vs storing full iters/valid sets).
                control_points_best=getattr(results_single_plan, "control_points_best", None),
                q_trajs_pos_best=getattr(results_single_plan, "q_trajs_pos_best", None),
                q_trajs_vel_best=getattr(results_single_plan, "q_trajs_vel_best", None),
                q_trajs_acc_best=getattr(results_single_plan, "q_trajs_acc_best", None),
            )
        torch.save(
